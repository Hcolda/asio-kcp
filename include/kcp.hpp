...
inline std::pair<std::string, unsigned short>
parse_host_port(const std::string &host_port, unsigned short default_port) {
...
public:
  using static_buffer_ptr_t =
      std::unique_ptr<static_buffer, static_buffer_deleter>;
...
class operation {
public:
  bool complete(void *owner, const std::error_code &ec,
                std::size_t bytes_transferred) {
    return func_(owner, this, ec, bytes_transferred);
  }

protected:
  using func_type = bool (*)(void *, operation *, const std::error_code &,
                             std::size_t);
...
class connection : public std::enable_shared_from_this<connection> {
  ...
  struct kcp_obj_deleter {
    void operator()(ikcpcb *p) { ikcp_release(p); }
  };
...
private:
  ...
  public:
    read_some_op(Handler &&handler, const MutableBuffer &buffer)
        : operation(&read_some_op::do_complete), handler_(std::move(handler)),
          buffer_(buffer) {}

    static bool do_complete(void *user, operation *base,
      ...
      if (ec) {
        ...
        int n = ikcp_recv(c->obj_.get(),
                          reinterpret_cast<char *>(op->buffer_.data()),
                          static_cast<int>(op->buffer_.size()));
        if (n > 0) {
          op->handler_(ec, n);
          return true;
        }
  ...
  public:
    read_op(Handler &&handler, StreamBuffer &buffer, size_t need)
        : operation(&read_op::do_complete), handler_(std::move(handler)),
          buffer_(buffer), need_(need) {}
    ...
    static bool do_complete(void *user, operation *base,
      ...
      if (ec) {
        ...
        while (true) {
          ...
          int n =
              ikcp_recv(c->obj_.get(), reinterpret_cast<char *>(buffer.data()),
  ...
  template <bool some> class initiate_async_read {
  public:
    typedef asio::any_io_executor executor_type;

    explicit initiate_async_read(connection *self) : self_(self) {}
    ...
    void operator()(ReadHandler &&handler, Buffer &&buffer, size_t need) const {
      if (nullptr != self_->read_op_ || self_->closed()) {
        ...
        asio::post(asio::bind_executor(
            executor, std::bind(std::forward<decltype(handler)>(handler),
                                self_->closed() ? asio::error::operation_aborted
                                                : asio::error::in_progress,
                                0)));
      } else {
        if constexpr (some) {
          using op_t =
              read_some_op<std::decay_t<ReadHandler>, std::decay_t<Buffer>>;
          self_->read_op_ = std::make_shared<op_t>(
              std::forward<ReadHandler>(handler), std::forward<Buffer>(buffer));
          asio::post(get_executor(),
                     [self_ = self_]() { self_->check_read_op(); });
        } else {
          using op_t = read_op<std::decay_t<ReadHandler>, std::decay_t<Buffer>>;
          self_->read_op_ =
              std::make_shared<op_t>(std::forward<ReadHandler>(handler),
                                     std::forward<Buffer>(buffer), need);
          asio::post(get_executor(),
                     [self_ = self_]() { self_->check_read_op(); });
        }
      }
  ...
  template <typename Handler> class write_op : public operation {
  public:
    write_op(Handler &&handler)
        : operation(&write_op::do_complete), handler_(std::move(handler)) {}
  ...
  public:
    ...
    explicit initiate_async_write(connection *self) : self_(self) {}
    ...
    void operator()(WriteHandler &&handler, Buffer &&buffer) const {
      if (nullptr != self_->write_op_ || self_->closed()) {
        ...
        self_->next_tick_ = 0;
        if (ikcp_send(self_->obj_.get(), (const char *)buffer.data(),
                      static_cast<int>(buffer.size())) < 0) {
          auto executor =
              asio::get_associated_executor(handler, self_->get_executor());
          asio::post(asio::bind_executor(
              executor, std::bind(std::forward<decltype(handler)>(handler),
                                  asio::error::message_size, 0)));
          return;
        }
        ...
        if (ikcp_waitsnd(self_->obj_.get()) <= idle_send_packet_count) {
          ...
          self_->write_op_ =
              std::make_unique<op_t>(std::forward<WriteHandler>(handler));
        }
      }
    }
  ...
  void init_kcp_context() {
    ...
    obj_->rx_minrto = 10;
    obj_->stream = 1;
    ikcp_setoutput(obj_.get(),
                   [](const char *buf, int len, ikcpcb *, void *user) {
  ...
  void start_timer() {
    ...
    timer_->async_wait(
        [this, self = shared_from_this()](const asio::error_code &e) {
          if (e) {
            return;
          }
          time_t now = clock();
...
public:
  ...
  connection(udp::socket *sock, uint32_t conv, udp::endpoint &endpoint,
             bool isserver)
      : isserver_(isserver), conv_(conv), now_tick_(clock()), sock_(sock),
  ...
  virtual ~connection() {
    ...
    console_log("%s.connection destructor: %u",
                (isserver_ ? "server" : "client"), conv_);
    if (!isserver_) {
      delete sock_;
    }
  }

  const executor_type &get_executor() noexcept { return sock_->get_executor(); }
   ...
   * sequence, until it's size() >=size.
   */
  template <typename Allocator, typename ReadHandler>
  auto async_read(asio::basic_streambuf<Allocator> &b, size_t size,
                  ReadHandler &&handler) {
    return asio::async_initiate<ReadHandler,
                                void(asio::error_code, std::size_t)>(
        initiate_async_read<false>(this), handler, asio::basic_streambuf_ref(b),
        size);
  }

  /* This function is used to asynchronously receive data from kcp. The function
  ...
  bool raw_send(const char *data, size_t size, uint8_t opcode) {
    ...
    if (opcode != opcode_disconnect) {
      sock_->async_send_to(
          const_buffer, endpoint_,
          [self = shared_from_this(),
           buffer = std::move(buffer)](const std::error_code &, size_t) {
  ...
  bool is_server() const { return isserver_; }

  udp::socket &get_socket() { return *sock_; }
  ...
  void close(asio::error_code ec) {
    ...
    if (ec == asio::error::timed_out) {
      console_log("connection(%s):  %u timeout",
                  (is_server() ? "server" : "client"), conv_);
    }
...
private:
  ...
  public:
    ...
    void operator()(AcceptHandler &&handler) const {
      ...
      self_->accept_ops_.push(
          std::make_shared<op>(std::forward<AcceptHandler>(handler)));
    }
  ...
  template <typename Handler> class accept_op : public operation {
  public:
    accept_op(Handler &&handler)
        : operation(&accept_op::do_complete), handler_(std::move(handler)) {}

    static bool do_complete(void *p, operation *base, const asio::error_code &,
                            std::size_t /*bytes_transferred*/) {
...
public:
  acceptor(const executor_type &executor, udp::endpoint endpoint,
           std::string magic)
      : magic_(magic), timer_(executor) {
  ...
  executor_type get_executor() { return timer_.get_executor(); }
...
private:
  asio::awaitable<void> do_update() {
    while (true) {
      ...
      auto [ec] =
          co_await timer_.async_wait(asio::as_tuple(asio::use_awaitable));
      if (!ec) {
        now_ = clock();
        for (auto iter = connections_.begin(); iter != connections_.end();) {
          auto lastrecvtime = iter->second->update(now_);
          if (iter->second->closed() ||
              (now_ - lastrecvtime) > timeout_duration) {
            iter->second->close(iter->second->closed()
  ...
  asio::awaitable<void> do_receive(udp::endpoint endpoint) {
    ...
    while (true) {
      auto [ec, n] = co_await socket.async_receive_from(
          asio::buffer(buffer.data(), buffer.size()), sender_endpoint,
      ...
      if (!ec) {
        ...
        if (opcode == opcode_handshark) {
          if (n == handshark_size &&
              std::string_view{buffer.data() + 1, magic_.size()} == magic_) {
            uint32_t conv = 0;
            memcpy(&conv, buffer.data() + 1 + magic_.size(), sizeof(uint32_t));
            asio::co_spawn(executor, handshark(socket, conv, sender_endpoint),
                           asio::detached);
          }
        } else {
          if (auto iter = connections_.find(sender_endpoint);
  ...
  asio::awaitable<void> handshark(udp::socket &sock, uint32_t conv,
...
inline asio::awaitable<std::tuple<asio::error_code, connection_ptr>>
async_connect(const Executor &executor, udp::endpoint endpoint,
              std::string magic) {
  thread_local static uint32_t conv = 0;
  ++conv;
  udp::socket sock(executor);
  auto [ec1] = co_await sock.async_connect(endpoint,
                                           asio::as_tuple(asio::use_awaitable));
  if (ec1) {
  ...
  auto [ec2, n] = co_await sock.async_send(asio::buffer(data),
                                           asio::as_tuple(asio::use_awaitable));
  if (ec2)
    co_return std::make_tuple(ec2, connection_ptr{});
  data.clear();
  data.resize(16, 0);
  auto [ec3, n2] = co_await sock.async_receive(
  ...
  if (opcode != opcode_handshark_response) {
    auto ec4 = moon::kcp::make_error_code(
        moon::kcp::error::invalid_handshark_response);
    co_return std::make_tuple(ec4, connection_ptr{});
  }
  memcpy(&conv, data.data() + 1, sizeof(conv));
  connection_ptr c = std::make_shared<connection>(
      new udp::socket(std::move(sock)), conv, endpoint, false);
  c->run_client();
