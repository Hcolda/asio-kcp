...
namespace std {
template <> struct is_error_code_enum<moon::kcp::error> {
  static bool const value = true;
};
template <> struct is_error_condition_enum<moon::kcp::condition> {
  static bool const value = true;
};
} // namespace std
...
public:
  const char *name() const noexcept override { return "moon.socket.kcp"; }

  std::string message(int ev) const override {
    switch (static_cast<error>(ev)) {
    default:
    case error::invalid_handshark_response:
      return "invalid_handshark_response";
...
public:
  ...
  std::string message(int cv) const override {
    ...
    default:
    case condition::kcp_handshake_failed:
      return "The kcp handshake failed";
    }
  }
};
} // namespace detail
...
inline std::error_code make_error_code(error e) {
  ...
  return std::error_code{static_cast<std::underlying_type<error>::type>(e),
                         cat};
}
...
inline std::error_condition make_error_condition(condition c) {
  ...
  return std::error_condition{
      static_cast<std::underlying_type<condition>::type>(c), cat};
}
} // namespace kcp
} // namespace moon